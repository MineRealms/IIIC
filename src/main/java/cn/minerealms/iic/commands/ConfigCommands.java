...
public class ConfigCommands {
    ...
    private static int showConfig(CommandContext<CommandSourceStack> ctx) {
        ...
        ctx.getSource().sendSuccess(() -> Component.literal(""), false);
        ctx.getSource().sendSuccess(() -> Component.literal("§d[GT Pollution]"), false);
        ctx.getSource().sendSuccess(() -> Component.literal(String.format("  §7Weight: §f%.2f", TriAxisConfig.gtPollutionWeight)), false);
        ctx.getSource().sendSuccess(() -> Component.literal(String.format("  §7Multiplier Base: §f%.2f", TriAxisConfig.gtSourceMultiplierBase)), false);
        ...
        ctx.getSource().sendSuccess(() -> Component.literal(""), false);
        ctx.getSource().sendSuccess(() -> Component.literal("§5[Hivemind Proximity]"), false);
